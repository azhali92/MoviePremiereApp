...
## Some Architecture related Keypoints

- In Home Tab two APIs are hit : **Popular Movies List** and **Genre** using **Promise.all**. The reason for this is because we **NEED** the Genre to eventually to display the Movie Detail when user clicks on the card and go inside. Without genre data, user will not have suffient data for the details screen.
- Genre response is cached and used throughout the App flow without hitting the API repeatedly
- There is NO API hit inside the movie details as most of the Data is passed from the Popular Movies List API via route param
